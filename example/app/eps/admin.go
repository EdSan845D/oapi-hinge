...
// ============ Enterpoint：管理（Children 挂载树演示） ============
//
// 注册逻辑说明（Children 与根挂载并存）：
//   - 产物仍是每个 owner 一个 Register{Owner}GinRow(i, k, ep)，i 为传入的根
//     router（main.go 里是 r.Group("/api")）；Children 只在生成期把挂载点
//     前缀烘焙进路由路径字符串，运行时没有嵌套 Group 结构。
//   - 本组两级挂载：/admin（AdminEp 挂载点）+ /audit（AuditEp 子节点挂载点）
//     → 最终路径 /api/admin/audit/events（/api 来自 main.go 的运行时根）。
...
//     挂载树与"默认全挂到根"两种形态可以自由并存。

// AdminEp 管理端点：Children 挂载树的根，组根 Enterpoint（纯挂载层演示）。
//
// oapi:tag 管理
type AdminEp struct {
	gen.EntryPoint
...
func (ep AdminEp) Index(ctx context.Context, _ any) (map[string]string, error) {
	return map[string]string{
		"module":  "admin",
		"message": "管理面板索引（Children 挂载树根）",
	}, nil
}

// AuditEp 审计端点：AdminEp 的 Children 子节点，挂载点 /audit 在 generate.go 声明。
//
// oapi:tag 审计
type AuditEp struct{}
